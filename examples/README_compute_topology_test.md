...
这组机器用于验证 MMCE N 的实体线路、无线骨干、分发塔吞吐和不完整算力分配。
...
1. 成型一台 `mmcen_demo_compute_tower`，并用 MMCE N 网络绑定器将矩阵控制器加入网络。
2. 在矩阵塔中运行 `mmcen_test_configure_compute_topology` 一次。
3. 成型四台测试机。每台的 `compute_port` 应自动归属本机控制器。
4. 用实体算力线缆连接供给机的端口、矩阵塔有线接口、分发塔和矩阵端口。
5. 启动供给机和有线消费机，确认两台机器只有在实体线路完整时推进。
6. 将无线消费机放入无线接口 32 格覆盖范围内。它不需要机器到接口的实体线缆，但无线接口到分发塔/矩阵必须有线连通。
7. 启动压力测试机，确认工作线路只有 96 CU/t 时不会因得到部分算力而推进。
8. 拆除任意一段线路，确认端点仍保留网络 ID，但对应算力路由变为离线。

压力测试机可以与有线消费机同时运行，用于验证同一接口的吞吐分配；也可以单独运行，用于验证 100 CU/t 超过 96 CU/t 时的完整需求检查。
