...
这组机器用于验证 MMCE N 的实体线路、无线骨干、分支交换节点吞吐和完整算力需求。

## 测试对象的固定定位

- **算力矩阵 MMCE 机器**：网络核心。结构内必须有 `compute_matrix_port`，并且矩阵控制器必须加入 MMCE N 网络。
- **算力供给机**：产生 CU/t。结构内放 `compute_port`，它不是矩阵，也不负责转发。
- **算力消费机 / 研究终端**：消耗 CU/t。结构内同样放 `compute_port`，它们只是网络用户。
- **算力线缆**：实体线路。它只负责证明两个端点确实连通。
- **分支交换节点**：可选线路设备。只有需要把机器分组、分流或单独限速时才使用。

测试时先完成最小有线网络，再单独加入分支交换节点和无线接入。这样每一步都能知道是哪个对象没有生效。
...
1. 成型一台 `mmcen_demo_compute_tower`，它是一个包含算力矩阵、分支交换节点、有线接入端和无线接入端的演示结构；用 MMCE N 网络绑定器将其控制器加入网络。
2. 在矩阵塔中运行 `mmcen_test_configure_compute_topology` 一次。
3. 成型四台测试机。每台的 `compute_port` 应自动归属本机控制器。
4. 用实体算力线缆把供给机的机器算力端口连接到矩阵结构的有线接入端，再连接到分支交换节点和算力矩阵。
5. 启动供给机和有线消费机，确认两台机器只有在实体线路完整时推进。
6. 将无线消费机放入无线接入端 32 格覆盖范围内。它不需要机器到接入端的实体线缆，但无线接入端到分支交换节点/算力矩阵必须有线连通。
7. 启动压力测试机，确认工作线路只有 96 CU/t 时不会因得到部分算力而推进。
8. 拆除任意一段线路，确认端点仍保留网络 ID，但对应算力路由变为离线。

压力测试机可以与有线消费机同时运行，用于验证同一线路或分支交换节点的吞吐分配；也可以单独运行，用于验证 100 CU/t 超过 96 CU/t 时不会以部分算力推进。
